...
const AddModal = ({handleAddModal}) => {
    ...
    const submitNewProduct = () => {
        if (state.name === '') {
            ...
            const URL = 'http://localhost:3001/uploadproduct'
    ...
    return <upLoadInfor.Provider value={contextValue}>
        <div className="fixed top-0 left-0 right-0 bottom-0 z-20">
            ...
            ></div>
            <div
                className="bg-slate-100 absolute top-20 bottom-20 left-[30%] right-[30%] max-w-[1380px] mx-auto p-4 shadow-xl">
                <form className={'flex flex-col justify-evenly'}>
                    <h1 className="text-2xl font-medium">Thêm sản phẩm </h1>
                    <form action="" className={'my-2'}>
                        <label className={'text-xl'} htmlFor="nameproduct">Ten san pham :</label>
                        <input id={'nameproduct'} placeholder={'Enter name product'} type="text"
                    ...
                    <div>
                        ...
                        <span className={'relative'}>
                            <textarea onChange={e => {
                                ...
                                dispatch({type: 'AddDescription', description: e.target.value})
                            }}
                                      className="w-full px-2 py-1 h-32 border-[1px]" placeholder="Mo ta san pham"
                                      maxLength={500}></textarea>
                        <span className={'absolute bottom-2 right-2 text-gray-400 text-sm'}>{length + '/' + 500}</span>
                        </span>
                    </div>
                    <div className={'flex-grow flex items-center justify-center'}>
                        <button className={'bg-sky-600 px-3 py-1.5 rounded-md'} onClick={e => {
                            e.preventDefault()
                            submitNewProduct()
                        }}>Them san pham
                        </button>
